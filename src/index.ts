import type { User, Course, Submission, ApiResponse, UserSummary, SubmissionUpdate, UserWithoutEmail, CourseCatalog, Item, Claim, ItemSummary } from "../types/index";
import { UserRole, ItemType, ClaimStatus } from "../types/index";
...
function createItem(data: Omit<Item, "id" | "reportedAt" | "isClaimed">): Item {
  const item: Item = {
    id: nextItemId++,
    reportedAt: new Date(),
    isClaimed: false,
...
function listItems(): ItemSummary[] {
  return items.map((it) => ({ id: it.id, title: it.title, type: it.type, location: it.location, isClaimed: it.isClaimed }));
}
...
function createClaim(itemId: number, claimerId: number, notes?: string): Claim {
  const item = getItemById(itemId);
  if (!item) throwError("Item not found");
  const claim: Claim = {
    id: nextClaimId++,
    itemId,
    claimerId,
...
function verifyClaim(claimId: number, verifierId: number): Claim {
...
// Sample usage
const backpack = createItem({ title: "Blue backpack", description: "With stickers", location: "Library", type: ItemType.Lost, reportedBy: student.id });
const umbrella = createItem({ title: "Black umbrella", description: "Plain", location: "Cafeteria", type: ItemType.Found, reportedBy: 2 });

console.log("Items:", listItems());
const claim1 = createClaim(backpack.id, 2, "I left this there yesterday");
console.log("Created claim:", claim1);
verifyClaim(claim1.id, 999); // 999 = admin verifier id (example)
console.log("After verification - items:", listItems());
